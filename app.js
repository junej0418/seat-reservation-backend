...
// Rate limiter 설정: 1분당 최대 30회 요청 제한
...
// ===== 개인정보 동의 여부 확인 API (신규 추가) =====
app.post('/api/privacy-consent/check', async (req, res) => {
  try {
    ...
    if(consent) {
      return res.json({success:true, hasConsent:true, consentDate:consent.consentDate});
    } else {
      return res.json({success:true, hasConsent:false});
    }
  } catch(e) {
    console.error('개인정보 동의 확인 실패:', e);
    res.status(500).json({success:false, message:'서버 오류'});
  }
});

// ===== 개인정보 동의 저장 API (신규 추가) =====
app.post('/api/privacy-consent', async (req, res) => {
  try {
    ...
    // 이미 동의한 사용자인지 확인
    const existing = await PrivacyConsent.findOne({roomNo, name, dormitory});
    if(existing) {
      return res.json({success:true, message:'이미 동의한 사용자입니다.', consent:existing});
    }
    ...
    await consent.save();
    console.log(`개인정보 동의 저장: ${name} (${roomNo}, ${dormitory}), IP: ${ipAddress}`);
    
    res.json({success:true, message:'개인정보 수집·이용에 동의하였습니다.', consent});
  } catch(e) {
    console.error('개인정보 동의 저장 실패:', e);
    if(e.code === 11000) {
      return res.json({success:true, message:'이미 동의한 사용자입니다.'});
    }
    res.status(500).json({success:false, message:'서버 오류'});
  }
});
...
// 예약 생성 또는 수정 API (개인정보 동의 확인 로직 추가)
app.post('/api/reservations', limiter, async (req,res) => {
  ...
  const consent = await PrivacyConsent.findOne({roomNo, name, dormitory});
  if(!consent) {
    return res.status(403).json({message:'개인정보 수집·이용에 먼저 동의해주세요.'});
  }
  // ===== 개인정보 동의 확인 끝 =====
