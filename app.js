...
const privacyConsentSchema = new mongoose.Schema({
  ...
  consentVersion: { type: String, default: '1.0' }, // 약관 버전
  ipAddress: { type: String },
  userAgent: { type: String }
});

// 인덱스: 룸번호+이름+기숙사 조합으로 유니크 (같은 사람 간주)
...
app.post('/api/check-privacy-consent', async (req, res) => {
  try {
    ...
    const consent = await PrivacyConsent.findOne({
      roomNo: roomNo,
      name: name,
      dormitory: dormitory
    });

    if (consent && consent.consentAgreed) {
      return res.json({ success: true, hasConsent: true });
    } else {
...
app.post('/api/save-privacy-consent', async (req, res) => {
  try {
    const { roomNo, name, dormitory, agreed } = req.body;
    ...
    if (!agreed) {
      return res.status(400).json({ success: false, message: '개인정보 이용에 동의해야 서비스를 이용할 수 있습니다.' });
    }
    ...
    console.log(`개인정보 동의 저장됨: ${name}(${roomNo}), 기숙사: ${dormitory}, 시간: ${new Date().toISOString()}, IP: ${ipAddress}`);

    res.json({ success: true, message: '개인정보 이용 동의가 저장되었습니다.' });
  } catch (e) {
