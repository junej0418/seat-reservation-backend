...
const reservationSchema = new mongoose.Schema({
  ...
  plainPassword: {type:String, required:true}, // 관리자 확인용 평문 비밀번호 (보안 유의)
...
app.post('/api/reservations', limiter, async (req,res)=>{
  ...
  try{
    ...
    let reservation;
    if(existingUser){ // 예약자 존재 시 비밀번호 검증 후 수정
      ...
      console.log(`[예약 변경 성공] ${reservation.name} (${reservation.roomNo}) 예약 변경됨.`);
    } else {
...
app.put('/api/reservations/update/:id', limiter, async (req,res)=>{
  ...
  try{
    ...
    const isSeatBooked = await Reservation.findOne({dormitory, floor, seat, _id: { $ne: id }});
    if(isSeatBooked)
      return res.status(409).json({message:'선택한 좌석은 이미 다른 사람에게 예약되었습니다.'});
...
app.delete('/api/reservations/:id', async (req,res)=>{
  try{
    ...
    const isPasswordCorrect = await bcrypt.compare(password, existingReservation.password);
    if(!isPasswordCorrect) return res.status(401).json({message:'예약 비밀번호가 일치하지 않습니다.'});
...
app.put('/api/announcement', async (req,res)=>{
  try{
    ...
    console.log(`공지사항 업데이트됨: 활성=${active}, 내용=${message.substring(0,30)}`);
...
app.post('/api/admin/reservations/:id/view-plain-password', async (req,res)=>{
  ...
  try{
    ...
    console.log(`관리자가 ${reservation.name} (${reservation.roomNo}) 비밀번호 열람함.`);
